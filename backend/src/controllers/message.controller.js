...
try {
    ...
    console.error("Error during getUsersForSideBar :", error.message);
...
export const getMessages =async (req,res) => {
    try {
        ...
        const messages = await Message.find({
            $or: [
              { senderId: myId.toString(), receiverId: userToChatId.toString() },
              { receiverId: myId.toString(), senderId: userToChatId.toString() }
            ]
          }).sort({ createdAt: 1 });
        ...
        console.error("Error during getMessages :",error.message);
        res.status(500).json({ error: "Internal server error" });
    }
    }
